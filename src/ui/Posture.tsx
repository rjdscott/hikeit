    ...
    <div>
      ...
      {railCrew.length === 0 ? (
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="tbl">
            <thead><tr><th>Posture</th><th className="num">arm vs sitting</th><th className="num">RM crew</th><th className="num">heel</th><th className="num">vs sitting</th><th className="num">free wind</th></tr></thead>
            <tbody>
              {ORDER.map((po) => {
                ...
                return (
                  <tr key={po} style={{ fontWeight: current === po ? 600 : 400 }}>
                    ...
                    <td className="num">+{fmt(POSTURE_OFFSET[po], 1)} m</td>
          ...
          <p className="small muted" style={{ marginTop: 6 }}>
            Full hike moves each rail crew's centre of gravity ~0.4 m further outboard than sitting — on a {fmt(2 * d.boat.halfbeamAt(7.5), 1)} m beam that is +{fmt((100 * POSTURE_OFFSET.hike) / (d.boat.halfbeamAt(7.5) - 0.35), 0)}% lever arm, for the same people. Same wind, same trim.
          </p>
        </div>
      )}
    </div>
